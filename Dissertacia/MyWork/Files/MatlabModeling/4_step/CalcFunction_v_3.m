function [ offset,angleOffset,x,y,z,insL,qtyCalcPoints,checkTime,stopCalc ] =...
    CalcFunction_v_3( time, velocityInsertion, velocityRotation,clcFlag,cF,fid)
...
if (time<=maxTime) % && dT_outOfModel>dT time<=maxTime qtyOfNumPoints <= numPoint
    ...
    for i = 1:modelSteps        
        ...
        %envF  = s*((rho*velocityInsertion*velocityInsertion)/2  );%K*(d - L0) + H*velocityInsertion;   envF  = cF*s*((rho*velocityInsertion*velocityInsertion)/2  );%K*(d - L0) + H*velocityInsertion;   
...
if (clcFlag == 1)
    velToString = string(velocityInsertion);
    %fileName = strcat(velToString,'_resulte.txt');
    %fid = fopen(fileName,'w');
    qtyOfNumPointsString = strcat(velToString,{' '},string(qtyOfNumPoints),'\n');
    %fwrite(fid,qtyOfNumPointsString);
    fprintf(fid,qtyOfNumPointsString);
    %fid = fclose(fid);
    [yP,zP] = CalcY_Z(rotationL,0,clcFlag);
    xP = 0;
    %yP = 0;
    %zP = 0;
    qtyOfNumPoints = 0;
    ...
    offset_YZ = 0;
    angleOffsetP = 0;    
    disp('clear All data insert insert');
end
